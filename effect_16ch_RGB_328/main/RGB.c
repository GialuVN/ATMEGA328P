...
extern volatile Color_pack RGB[max_led];
...
void RGB_out_to_lpd(Color_pack *rgb, uint16_t nled)
{
	...
	{
		data[i*3]=rgb[i].Color.Red;
		data[i*3+1]=rgb[i].Color.Green;
		data[i*3+2]=rgb[i].Color.Blue;
	}
	output_updata(data);
}
	...
	switch (style)
	{
		...
		for (uint16_t j = 0; j <loop; j++)
		{
			do
			{
				...
				RGB_rotatal(rgb,first_point,last_point,1);
				RGB_out_to_lpd(rgb,_num+1);
				for (uint8_t i = 0; i<speed;i++) _delay_ms(1);
			}
			while (RGB_color_change(&cur_color,&to_color,div)==0);
			cur_color = cl2;
			to_color =  cl1;
		...
		for (uint16_t j = 0; j <loop; j++)
		{
			do
			{
				...
				RGB_rotatal(rgb,first_point,last_point,0);
				RGB_out_to_lpd(rgb,_num+1);
				for (uint8_t i = 0; i<speed;i++) _delay_ms(1);
			}
			while (RGB_color_change(&cur_color,&to_color,div)==0);
		...
		for (uint16_t j = 0; j <loop; j++)
		{
			do
			{
				rgb[last_point] = cur_color;
				rgb[first_point] = cur_color;
				RGB_rotatal(rgb,first_point,((last_point-first_point)/2)+first_point,0);
				RGB_rotatal(rgb,((last_point-first_point)/2)+first_point+1,last_point,1);
				RGB_out_to_lpd(rgb,_num+1);
				for (uint8_t i = 0; i<speed;i++) _delay_ms(1);
			}
			while (RGB_color_change(&cur_color,&to_color,div)==0);
		...
		for (uint16_t j = 0; j <loop; j++)
		{
			do
			{
				...
				RGB_rotatal(rgb,first_point,((last_point-first_point)/2)+first_point,1);
				RGB_rotatal(rgb,((last_point-first_point)/2)+first_point+1,last_point,0);
				RGB_out_to_lpd(rgb,_num+1);
				for (uint8_t i = 0; i<speed;i++) _delay_ms(1);
			}
			while (RGB_color_change(&cur_color,&to_color,div)==0);
...
{
		uint16_t _num = last_point - first_point;
		Color_pack cur_color =RGB_base_convert((random()%7));
		Color_pack to_color = RGB_base_convert((random()%7));
		switch (style)
		{
			case 0:
			for (uint16_t j = 0; j <loop; j++)
			{
				...
				while (RGB_color_change(&cur_color,&to_color,div)==0);
				cur_color = to_color;
				to_color = RGB_base_convert((random()%7));
			}
			break;
...
void RGB_color_change_sys(Color_pack *rgb,uint16_t first_point,uint16_t last_point,Color_pack to_color,uint8_t speed,uint8_t ratio,uint16_t wait)
{
	...
	do 
	{
		...
		RGB_out_to_lpd(rgb,(last_point-first_point)+1);
		for (uint16_t i = 0;i<speed;i++) _delay_ms(1);
	} while (RGB_color_change(&_cur_color,&to_color,ratio)==0);
...
void RGB_paint_touch(Color_pack *rgb,uint16_t first_point,uint16_t last_point,Color_pack _color,uint8_t style)
{
	...
	{
		case 0:
		to_color = RGB_pack_convert(0,0,0);
		for (uint16_t i = 0; i <(_num+1); i++)
		{
			cur_color = _color;
			RGB_color_change(&cur_color,&to_color,round(k*i));
		...
		for (uint16_t i = 0; i <(_num+1); i++)
		{
			cur_color =RGB_pack_convert(0,0,0);
			RGB_color_change(&cur_color,&to_color,round(k*i));
			rgb[first_point+i].Color.Red |= cur_color.Color.Red;
			rgb[first_point+i].Color.Green |= cur_color.Color.Green;
			rgb[first_point+i].Color.Blue |= cur_color.Color.Blue;
...
{
	RGB_color_to_color(rgb,first_point,(last_point-first_point)/2+first_point,color1,color2,0);
	RGB_color_to_color(rgb,(last_point-first_point)/2+1+first_point,last_point,color2,color1,0);	
}