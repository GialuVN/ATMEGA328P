...
//	uint8_t counter = 0;
	...
	for (uint8_t i = 0;i<=(top-bot+1);i++) flag[i] = random()%2;
	for (uint8_t i = 0;i<=(top-bot+1);i++) time_sleep[i] =random()%max_sleep;
	//while (1)
	//{
			//counter = 0;
		//for (uint8_t i = 0;i<(top-bot+1);i++)
