...
class Adafruit_VS1053_FilePlayer : public Adafruit_VS1053 {
 public:
  ...
  boolean IsPlayable(char* File_Name);
  void PlayRoot(void);
  File PlayNext(void);
  
  
  void music_path(File dir);
  void acc_close(void);
  void acc_reset(uint16_t acc_size);
  
  
 //void Make_list(File *dir);
 private:
 int16_t acc_Rewind(void);
 int16_t acc_Next(const char *c);
 File _file_save;
 long _position;
 char *_path;
 uint16_t _numchar;
