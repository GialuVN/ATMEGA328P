...
void pwm_phasecorrect_timer1_option_initial(unsigned char clk,unsigned char nbits)
{
	DDRB |=(1<<PORTB2)|(1<<PORTB1); // OC1A,OC1B IS OUTPUT
	TCCR1A = (0<<COM1A0)| (1<<COM1A1)| (0<<COM1B0)| (1<<COM1B1)|nbits;
	TCCR1B =(clk<<CS00);
	TCNT1 = 0;
	OCR1A = 0;
	OCR1B = 0;
...
void pwm_phasecorrect_4ch_updata(unsigned char ch1,unsigned char ch2,unsigned char ch3,unsigned char ch4)
{
	...
	pwm_phasecorrect_timer2_OCR2A_updata(ch1);
	pwm_phasecorrect_timer0_OCR0A_updata(ch2);
	pwm_phasecorrect_timer0_OCR0B_updata(ch3);
	pwm_phasecorrect_timer2_OCR2B_updata(ch4);
}

void pwm_servo_2ch_updata(unsigned int ch1,unsigned int ch2)
{
	pwm_servo_timer1_OCR1A_updata(ch1);
	pwm_servo_timer1_OCR1B_updata(ch2);
